...
def mettre_a_jour_listbox():
    ...
    cursor.execute('SELECT id, date_candidature, nom_societe, reponse_societe FROM recherches_stages')
    resultats = cursor.fetchall()
    
    titres = ["ID", "Date de Candidature", "Nom Société", "Réponse Société"]
...
def afficher_fenetre_ajout_recherche():
    ...
    entry_cv_envoyes_ajout = Entry(fenetre_ajout)
    entry_cv_envoyes_ajout.grid(row=1, column=1)
    ...
    entry_lien_recrutment_ajout.grid(row=4, column=1)
...
def ajouter_recherche_from_window(fenetre_ajout, cv, lettre, nom_societe, lien_recrutment):
    ...
    cursor.execute('''
        INSERT INTO recherches_stages (date_candidature, cv_envoyes, lettre_envoyes, nom_societe, lien_recrutment, reponse_societe)
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (date_candidature, cv, lettre, nom_societe, lien_recrutment, None))
    conn.commit()
    messagebox.showinfo("Succès", "Recherche de stage ajoutée avec succès")
    mettre_a_jour_listbox()  # Mettre à jour la liste après l'ajout
...
listbox_recherches = Listbox(fenetre, height=10, width=90)
